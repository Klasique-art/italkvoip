# Fix package imports for iTalkVoIP - Enhanced Version
# Run this from the project root directory

Write-Host "==================================================" -ForegroundColor Green
Write-Host "  Fixing ALL package imports for iTalkVoIP" -ForegroundColor Green
Write-Host "==================================================" -ForegroundColor Green
Write-Host ""
...
Write-Host "[1/4] Fixing 'import org.linphone.R' -> 'import com.italkvoip.app.R'" -ForegroundColor Yellow
$count = 0
Get-ChildItem -Path "app\src" -Recurse -Include *.kt | ForEach-Object {
    ...
    if ($content -and $content -match 'import org\.linphone\.R\b') {
        $newContent = $content -replace 'import org\.linphone\.R\b', 'import com.italkvoip.app.R'
...
Write-Host "[2/4] Fixing 'import com.italkvoip.app.BuildConfig' -> 'import com.italkvoip.app.BuildConfig'" -ForegroundColor Yellow
$count = 0
Get-ChildItem -Path "app\src" -Recurse -Include *.kt | ForEach-Object {
    $content = Get-Content $_.FullName -Raw -ErrorAction SilentlyContinue
    if ($content -and $content -match 'import org\.linphone\.BuildConfig') {
        $newContent = $content -replace 'import org\.linphone\.BuildConfig', 'import com.italkvoip.app.BuildConfig'
        Set-Content $_.FullName -Value $newContent -NoNewline
        Write-Host "  ✓ Fixed: $($_.Name)" -ForegroundColor Cyan
        $count++
    }
...
Write-Host "[3/4] Fixing 'import com.italkvoip.app.databinding' -> 'import com.italkvoip.app.databinding'" -ForegroundColor Yellow
$count = 0
Get-ChildItem -Path "app\src" -Recurse -Include *.kt | ForEach-Object {
    $content = Get-Content $_.FullName -Raw -ErrorAction SilentlyContinue
    if ($content -and $content -match 'import org\.linphone\.databinding') {
        $newContent = $content -replace 'import org\.linphone\.databinding', 'import com.italkvoip.app.databinding'
        Set-Content $_.FullName -Value $newContent -NoNewline
        Write-Host "  ✓ Fixed: $($_.Name)" -ForegroundColor Cyan
        $count++
    }
...
Write-Host "[4/4] Fixing 'type=`"org.linphone' -> 'type=`"com.italkvoip.app' in XML files" -ForegroundColor Yellow
$count = 0
